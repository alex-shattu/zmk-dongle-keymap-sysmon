...
- Распиновка повторяет [Snake Dongle](https://github.com/joaopedropio/snake-dongle),
  поэтому корпуса под него подходят. Подсветка на `D8`, а не наглухо на `VCC`, —
  именно это позволяет ею управлять.
...
- Число пинов у модулей разных продавцов отличается, как и наличие выведенного
  ...
  (берите вариант «WITH Backlight Control» — эта прошивка управляет подсветкой);
  таблица выше — та же распиновка текстом.
