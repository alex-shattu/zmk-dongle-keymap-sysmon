...
(входит в комплект, для macOS и Linux); без него в её строках стоит `--`.

> [!IMPORTANT]
> **Подписи клавиш пишутся руками, и в репозитории лежит кеймап автора.** ZMK не
> хранит печатных подписей для биндингов в рантайме, поэтому вывести их из
> вашего `.keymap` невозможно. Они берутся из таблицы на C —
> [`keymap_legend.c`](boards/shields/dongle_tft/keymap_legend.c), — в которой
> сейчас шестислойная раскладка Charybdis 5×3+3.
>
> Таблица компилируется внутрь модуля, и ничто в вашем zmk-config её не
> переопределит, так что **рассчитывайте на форк репозитория**, а не на ссылку
> из `west.yml` на этот: сборка без правок выведет вам на экран чужие подписи.
> Дальше таблица ваша, и синхронизировать её с кеймапом придётся вручную —
> следить за ним автоматически некому.
>
> Два способа ошибиться из трёх ломают сборку, а не картинку: таблица, чья
> геометрия расходится с `CONFIG_DONGLE_TFT_*`, и конфигурация, расходящаяся с
> вашим `zmk,physical-layout`. Третий молчаливый — слой, который есть в кеймапе
> и которого нет в таблице, рисуется пустой сеткой.
>
> Правила — в разделе [Как приспособить под свой кеймап](#как-приспособить-под-свой-кеймап).
> Всё остальное на экране — строка статуса, модификаторы, заряд, вся нижняя
> половина — ничего этого не требует и работает как есть.
...
В `config/west.yml` своего конфиг-репозитория добавьте репозиторий как проект.
Указывайте **свой форк**, а не этот: таблица подписей живёт внутри модуля, так
что править её — значит владеть той копией, которую тянет west:
...
Пуш — и скачивайте артефакт из прогона Actions. Это вся обвязка: шилд приносит
свои дефолты Kconfig (глубина цвета, буферы LVGL, кастомный status screen,
второй USB-serial). Чего он **не** приносит — так это ваших подписей, см.
[Как приспособить под свой кеймап](#как-приспособить-под-свой-кеймап).
...
HID-интерфейсами. В комплекте два демона на Python: дважды в секунду они снимают
CPU, память, диск и сеть и пишут по строке на выборку — в одном и том же формате,
так что прошивка не различает, с каким хостом говорит:

| Демон | Автозапуск | Примечания |
| ----- | ---------- | ---------- |
| [`host/sysmon-daemon/`](host/sysmon-daemon/) (macOS) | launchd-агент | зовёт `pmset`, `route` и `osascript`; на Apple Silicon температуры CPU нет |
| [`host/sysmon-daemon-linux/`](host/sysmon-daemon-linux/) (Linux) | пользовательский юнит systemd | читает только `/proc` и `/sys`, без подпроцессов; нужно udev-правило, чтобы ModemManager не занимал порт |

У каждого свой README ([macOS](host/sysmon-daemon/README.ru.md),
[Linux](host/sysmon-daemon-linux/README.ru.md)): установка, юнит автозапуска,
откуда берётся каждое число и описание протокола.

Протокол намеренно примитивный (одна строка текста с разделителем `|`, handshake
`PING` → `SYSMON1`, чтобы демон нашёл нужный порт), так что демон для третьей ОС —
...
- По 36 записей на слой, в порядке биндингов: три ряда 5+5, затем 3+3 больших
  пальца. Слои — в том же порядке, что в вашем узле keymap, и **по строке на
  каждый ваш слой**: число слоёв — единственное, что не проверяется на сборке,
  так что недостающий слой нарисуется пустой сеткой.
