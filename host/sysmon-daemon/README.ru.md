...
Для Linux есть [`../sysmon-daemon-linux/`](../sysmon-daemon-linux/README.ru.md) —
тот же формат на проводе, systemd вместо launchd.
...
диалог фоновому агенту толком негде. Симптомы — `--` на дисплее, а в логе:
...
Прошивка принимает и старые S1/S2 (конвертируя total-поля), и S3; демон всегда
шлёт S3. `net_iface`, `temp_c` и `thermal_state` разбираются, но не рисуются — в
нижней половине донгла для них нет места.
