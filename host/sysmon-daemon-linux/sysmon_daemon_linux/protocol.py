...
Byte-for-byte the same protocol the macOS daemon speaks — the firmware sees
no difference between the two hosts. Kept as a copy rather than a shared
package so each daemon stays a single self-contained directory you can drop
onto a machine.
...
- net_up_kbps / net_down_kbps / disk_used_gb / disk_free_gb: non-negative,
  ...
  disk values are DECIMAL GB (bytes / 1e9), i.e. what `df --si` shows;
